...
def country_clans(code, sort):
    if clans := list(Clan.objects(location__countryCode=code.upper(), active_members__gte=10).order_by(ORDER_MAPPING[sort]).limit(50)):
        title = f"{clans[0].location['name']}'s {TITLE_MAPPING[sort]}"
        return render_template("country.html", clans=clans, title=title, sort=sort, code=code)
    else:
